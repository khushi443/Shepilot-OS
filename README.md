...
`npm run dev` starts plain Vite and serves the React app at
`http://localhost:5173` — but it does **not** run the `/api/generate`
serverless function, so AI generation will fail with network/404 errors
under plain `npm run dev`.

To test AI generation locally, run the app through the Vercel CLI instead,
which emulates the `/api` routes:
```bash
npm install -g vercel   # one-time
vercel dev
```
Set `OPENROUTER_API_KEY` (and optionally `OPENROUTER_MODEL`) in a local
`.env` file first — see `.env.example`. `vercel dev` will prompt to link
the project the first time you run it.
