...
// Configurable via Vercel env var so a bad/retired model can be swapped
// without a code change and redeploy-only fix.
...
function errorMessageForStatus(status, error) {
  ...
  switch (status) {
    case 400:
      // OpenRouter returns 400 for a malformed request or an unknown/retired
      // model slug — surface the upstream detail so this doesn't collapse
    ...
    default:
      return upstreamMessage || "Unable to generate AI response. Please try again.";
  }
}
